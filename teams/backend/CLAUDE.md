# Backend Team — Workspace Mandate
...
4. **Test.** Follow the tdd skill: add or update tests for the behavior you
   changed, then run the suite and show the output. A backend change must also
   run the frontend and end-to-end suites when they exist — a change in one
   layer has to prove it did not break the others. If a check fails, fix it
