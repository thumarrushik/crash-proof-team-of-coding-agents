...
![A vertical timeline of the PR #4 incident, six boxes. A slate box: two backend PRs collide on the same lines, the search PR merges first and PR #4 (delete) goes stale. A red box: PR #4 is approved on its merits but the merge is refused, the branch is behind. A red box: the update-the-branch fix hits a content conflict, keeping both is a judgment call. A dashed arrow, escalated across lanes, to a purple box: handed to the team that owns the code, a resolve job in the backend namespace via a client call from inside an activity. An amber box: the backend agent keeps both features, tests ten of ten green, the harness merges #4 and the issue closes itself. A final dashed arrow, the closed issue frees the blocked work, to a green box: the blocked frontend issue is released and ships a delete button and browser test, no human decision in the whole chain](../../assets/diagrams/conflict-timeline.png)
