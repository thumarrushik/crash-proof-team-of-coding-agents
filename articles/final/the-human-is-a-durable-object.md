...
[^decide]: The decision is a workflow update, `decide`, with a validator that rejects an update when no gate is open or when the decider is unnamed. The rejection happens before anything enters history; the update handler and its validator are in the [evidence repository](https://github.com/thumarrushik/crash-proof-team-of-coding-agents).
[^gate]: The wait is `workflow.wait_condition(lambda: decision is not None, timeout=...)`; on `asyncio.TimeoutError` the workflow synthesizes a denial attributed to `deadline`. The gate helper is in the [evidence repository](https://github.com/thumarrushik/crash-proof-team-of-coding-agents).
