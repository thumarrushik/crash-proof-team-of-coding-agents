...
*The kill, as witnessed. No mechanism in the frame: it came back, and the how is the first half of this article.*
...
## Two Ways We Nearly Fooled Ourselves

Our headline kill was a **single chunk**: the agent was mid-run, no chunk had finished, so nothing was saved yet and the session ID lived only in the last heartbeat. That makes it the hardest test of the recovery, with no completed boundary to fall back on. It counts as real only if two things hold, and each is a caveat worth stealing.

- **The last heartbeat has to actually reach the server.** A running step does not ship every pulse the instant it fires; the SDK batches them on a throttle, and the default is too slow for a short chunk. Kill before a pulse is saved and the session ID is simply gone. It survives here only because this project tightens that throttle on purpose.⁸ Miss the window and, with no finished chunk behind it, the run starts fresh: slower, never stuck.
- **The whole process group has to die, not just the worker.** A chunk is not one process. The worker launches Claude Code as a separate child process to do the coding. Kill only the worker and that child is orphaned: it keeps running, finishes the task, and writes its result anyway, so the job looks recovered when the original attempt never actually died. Our first demos passed exactly this way and were lying before we caught them. Kill the whole group at once, and a clean finish can only mean a fresh attempt resumed the session.

(A third trap only appears once a job spans several chunks: the kill must land during a chunk, not in the gap just after a finished one, or you are watching ordinary durable continuation and mistaking it for the heartbeat rescue. Our single chunk has no such gap.)
...
- **A permanently dead machine takes the workspace with it.** The transcript and working directory are local files, so a host that never comes back loses them. Restarts are covered by the sticky-queue pinning below; machine-loss wants shared storage, and a two-filesystem run has since proven the resume across it: machine A's disk deleted, machine B finishing the same session from the heartbeat ID ([How It's Built](how-its-built.md) has the mechanism and the receipt). What the harness still does not ship is the sync itself.
...
9. The child-kill run, 2026-08-30, model `haiku`, lane `testing`: only the Claude CLI subprocess was SIGKILLed mid-chunk while its worker stayed up. The SDK surfaced the dead CLI at once, the activity raised a retryable error, and attempt 2 resumed the *same* session from heartbeat details (input session ID null) about five seconds later, the retry policy's initial backoff; the worker process was never restarted. This is the same resume-from-heartbeat path as a whole-worker crash, caught by a raised error instead of the two-minute liveness timeout. Runner and evidence: `deploy/child-kill-recovery.sh` and `deploy/child-kill-recovery-results.md`.
10. The four-cost experiment, model `haiku`, 2026-07-15, run strictly sequentially, everything observed rather than modeled; the method and per-run detail are in the economics companion, [*Mechanics Cost Cents, Behavior Costs Dollars*](mechanics-cost-cents.md).
11. The asterisk: PR #4 predated the escalation feature, so its review was re-triggered against the new code, and this run's polls were hand-triggered rather than waited out on the timer. The scheduled poll and the first-pass routing are proven across the other recorded runs; this run isolates the resolution chain. The full run record is in the evidence repository.
12. [*Dive into Claude Code*](https://arxiv.org/abs/2604.14228) (arXiv 2604.14228): ~1.6% of the codebase is AI decision logic; ~98.4% operational harness. *Practitioner/academic.*
13. [Temporal AI cookbook](https://docs.temporal.io/ai-cookbook/agentic-loop-tool-call-claude-python): the "Basic agentic loop with Claude" uses model Messages API calls as activities; no Claude Code CLI, no session resume. *Vendor/canonical.*
14. ["Building fault-tolerant long-running AI workflows with Claude Agent SDK × Temporal.io"](https://claudelab.net/en/articles/api-sdk/claude-agent-sdk-temporal-durable-ai-workflows-production-guide), claudelab.net (2026-04-22): wraps API messages in activities; no headless sessions, no cross-activity resume. *Practitioner.*
15. ["Don't make Claude do the same work twice"](https://www.zenml.io/blog/claude-agent-sdk-durable-runtime), zenml.io (2026-06-01): durable checkpointing of Claude Agent SDK runs on ZenML's own runtime, not Temporal. *Practitioner.*
