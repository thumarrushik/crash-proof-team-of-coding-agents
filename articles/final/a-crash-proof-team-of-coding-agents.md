...
[^childkill]: The child-kill run, 2026-08-30, model `haiku`, lane `testing`: only the Claude CLI subprocess was SIGKILLed mid-chunk while its worker stayed up. The SDK surfaced the dead CLI at once, the activity raised a retryable error, and attempt 2 resumed the *same* session from heartbeat details (input session ID null) about five seconds later, the retry policy's initial backoff; the worker process was never restarted. This is the same resume-from-heartbeat path as a whole-worker crash, caught by a raised error instead of the two-minute liveness timeout. Runner and evidence: `deploy/child-kill-recovery.sh` and `deploy/child-kill-recovery-results.md`.
