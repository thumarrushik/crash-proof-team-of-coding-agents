...
Two different failures, one recovery: read the last session ID from the heartbeat and resume, never restart. Only the alarm differs. A dead worker goes silent, and a liveness timeout catches it in about two minutes; a dead agent process throws an error the activity catches in seconds. We killed both, and both finished as the same session.[^7][^childkill]
