...
- **Worker affinity.** The transcript and workspace are local files: the engine can recover the *workflow* anywhere, but a retried chunk has to land where the *session* lives. The sticky-queue fix below handles the common case, a worker that restarts on the same host, and it has now been run live: two worker identities on a real server, the pinned one killed mid-run, the survivor never stealing the pinned work, and a restarted same-identity process resuming the same session to completion. The honest edge remains: both identities shared one filesystem, so a *permanently* dead machine — its local files gone with it — still wants shared storage for the transcript and workspace, and that two-filesystem run is the receipt still owed ([How It's Built](how-its-built.md) has the mechanism).
