...
- **Deny `git push` to every agent.** The agent writes code but can never reach the remote. When work needs pushing or merging, a separate harness activity does it, with its own credentials, as a recorded step in the durable history. No agent holds a world-changing credential, and every outward change has an audit entry.
