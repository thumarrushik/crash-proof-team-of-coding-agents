...
[^merge]: The merge switch: the review workflow reads `tests_passed = bool(report.get("tests_passed"))` and merges on it (plus the optional human gate, the companion [The Human Is a Durable Object](the-human-is-a-durable-object.md)); the review prompt instructs the agent to set the field; the workflow and poller code are in the evidence archive. The main article's merge-gate limitation names this a placeholder.
